...
Combines Supabase VIP analysis + SaneBox filtering + Lindy AI processing + Missive integration
"""
...
class IntelligentEmailProcessor:
    ...
    def load_vip_analysis(self):
        ...
        try:
            with open('/tmp/vip_analysis_complete.json', 'r', encoding='utf-8') as f:
            ...
            self.logger.info(f"Loaded VIP data: {len(self.vip_emails)} contacts, {len(self.urgency_keywords)} urgency keywords")
    ...
    def classify_email(self, sender: str, subject: str, content: str) -> EmailProcessingResult:
        ...
        urgency_detected, urgency_score = self._detect_urgency(subject, content)
        
        # Step 3: Content Classification
        classification = self._classify_content(subject, content, vip_status, urgency_detected)
        
        # Step 4: Priority Calculation
        priority_level = self._calculate_priority(vip_status, urgency_score, classification, vip_contact)
        
        # Step 5: Response Generation
        suggested_response = self._generate_response_suggestion(
            sender, subject, content, classification, vip_status, urgency_detected
        )
        
        # Step 6: Confidence Assessment
        confidence = self._assess_confidence(vip_status, urgency_detected, classification)
    ...
    def _detect_vip_status(self, sender: str) -> tuple[bool, Optional[Dict]]:
    ...
    def _detect_urgency(self, subject: str, content: str) -> tuple[bool, float]:
        ...
        crisis_words = ['emergency', 'broken', 'nefunguje', 'kritické', 'notfall', 'asap']
        for word in crisis_words:
            if word in text:
                urgency_score += 0.3
        
        # Time-sensitive words
        time_words = ['today', 'dnes', 'heute', 'immediately', 'okamžitě', 'sofort']
        for word in time_words:
            if word in text:
                urgency_score += 0.2
        
        urgency_detected = urgency_score >= self.thresholds['urgency_confidence_minimum']
        
        return urgency_detected, min(urgency_score, 1.0)
    
    def _classify_content(self, subject: str, content: str, is_vip: bool, is_urgent: bool) -> str:
        """Classify email content type"""
        text = f"{subject} {content}".lower()
        
        # Business classifications
        if any(word in text for word in ['invoice', 'faktura', 'payment', 'platba', 'bill']):
            return 'financial'
        
        if any(word in text for word in ['meeting', 'call', 'schůzka', 'kalendář', 'calendar']):
            return 'meeting_request'
        
        if any(word in text for word in ['order', 'objednávka', 'delivery', 'dodávka']):
            return 'order_inquiry'
        
        if any(word in text for word in ['quote', 'nabídka', 'price', 'cena']):
            return 'quote_request'
        
        if any(word in text for word in ['problem', 'issue', 'broken', 'problém', 'nefunguje']):
            return 'support_request'
        
        if any(word in text for word in ['thank', 'thanks', 'děkuji', 'danke']):
    ...
    def _calculate_priority(self, is_vip: bool, urgency_score: float, classification: str, vip_contact: Optional[Dict]) -> int:
    ...
    def _generate_response_suggestion(self, sender: str, subject: str, content: str, 
                                    classification: str, is_vip: bool, is_urgent: bool) -> str:
...
Thank you for your financial inquiry. I'm reviewing the details immediately.
...
Thank you for reporting this issue. I understand the urgency and am addressing it immediately.

I'll provide an update within 1 hour. If this is critical, please call me directly.
...
Thank you for your meeting request. I'm checking my calendar and will confirm available time slots within 2 hours.
...
Thank you for your order inquiry. I'm checking availability and delivery options immediately.
...
You'll receive the quote within 4 business hours. If you have any specific requirements, please let me know.
...
Thank you for your message. As a valued client, I'm prioritizing your inquiry.
...
Thank you for your inquiry. I'm reviewing your message and will provide a detailed response within 4 business hours.
...
Premium Gastro"""
        ...
        base_response = templates.get(classification, templates['general_inquiry'])
        
        # Add urgency note if urgent
        if is_urgent:
            base_response = base_response.replace("Best regards,", "URGENT PRIORITY - Best regards,")
        
        # Add VIP note if VIP
        if is_vip:
            base_response = base_response.replace("Thank you for", "Thank you for your valued")
        
        return base_response
    
    def _assess_confidence(self, is_vip: bool, is_urgent: bool, classification: str) -> float:
        ...
        high_confidence_classes = ['financial', 'support_request', 'meeting_request']
    ...
    def process_email_batch(self, emails: List[Dict]) -> List[EmailProcessingResult]:
    ...
    def export_processing_rules(self) -> Dict:
        ...
        top_vip_emails = [
            contact['email'] for contact in self.vip_data['vip_contacts'][:50]
            if contact['vip_score'] >= self.thresholds['high_priority_minimum']
        ]
        ...
        return {
            ...
            'lindy_processing_rules': {
                'process_only_folders': ['Inbox', '@SanePriority', '@SaneMoney', '@SaneMeetings'],
...
def main():
    ...
    test_emails = [
        {
            ...
            'content': 'We need immediate payment for invoice #12345. This is overdue and requires attention.'
        },
        {
            'sender': 'vanduch@montycon.cz', 
            'subject': 'Meeting request for next week',
            'content': 'Could we schedule a meeting to discuss the new menu items?'
        },
        {
        ...
        },
        {
            ...
            'content': 'Our guests are waiting and the delivery is not here. Please help ASAP!'
        }
    ]
    ...
    for i, result in enumerate(results, 1):
        ...
        print(f"\n{i}. Priority {result.priority_level}/10 | {vip_status} | {urgency_status}")
        print(f"   From: {result.sender}")
        print(f"   Subject: {result.subject}")
        print(f"   Classification: {result.classification}")
        print(f"   Confidence: {result.processing_confidence:.1%}")
        print(f"   Suggested Action: {result.suggested_response.split('.')[0]}...")
    ...
    print(f"📁 Processing rules exported to: /tmp/email_processing_rules.json")
